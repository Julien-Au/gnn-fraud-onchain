...
scaling): PR-AUC $0.469 \rightarrow 0.966$ ($+0.497$).
\todo{add multi-seed hetero numbers (batch2: seeds 43, 44)}
...
\begin{tabular}{lcc}
\toprule
Component & GraphSAGE & XGBoost \\
\midrule
Base rate & $+0.026\pm0.005$ & $+0.005\pm0.001$ \\
Message-passing leakage & $-0.000\pm0.007$ & n/a \\
Distribution access & $+0.384\pm0.044$ & $+0.189\pm0.002$ \\
\midrule
Total gap & $+0.410\pm0.041$ & $+0.195\pm0.001$ \\
\bottomrule
\end{tabular}
\end{table}

The message-passing channel we ourselves hypothesized is \textbf{null}: training
with test nodes entirely removed from the graph changes random-split PR-AUC by
$-0.000\pm0.007$ (0.894 versus 0.894). The dominant term is distribution access:
a random split trains the model on examples drawn from the test period, an
...
\todo{add GCN and GAT decomposition columns (batch2)}
...
where the distribution does not shift, there is nothing to access. Caveats stated
plainly: ROC-AUC does inflate ($+0.031$), both arms sit near the PR-AUC floor, and
GADBench reports strong GNNs on DGraph-Fin, so this control is supporting evidence
rather than proof. \todo{add hidden-dim-128 control run (batch2)}
...
PR-AUC $\sim$0.01) and only partially recovers; a learning-rate
$\times$ gradient-clipping sweep does not change the conclusion.
\todo{cite evolvegcn\_sweep.json artifact numbers (batch2)}
